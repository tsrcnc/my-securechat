...
export async function sendPasswordResetEmail(email: string, token: string) {
    if (!process.env.RESEND_API_KEY) {
        ...
        console.log(`[DEV MODE] Password reset for ${email} - Token: ${token}`);
        return;
    }
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: 'My SecureChat <auth@noreplay.mysecurechat.org>',
            to: [email],
            subject: 'Reset Your Password - My SecureChat',
            html: `
                <!DOCTYPE html>
                <html>
                <head>
                                    ...
                                    <tr>
                                        <td style="padding: 40px 30px;">
                                            ...
                                            <table width="100%" cellpadding="0" cellspacing="0">
                                                <tr>
                                                    <td align="center" style="padding: 20px 0;">
                                                        <a href="${resetLink}" style="display: inline-block; background-color: #3b82f6; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">Reset Password</a>
                                                    </td>
                                                </tr>
                                            </table>
                                            
                                            <p style="color: #6b7280; margin: 30px 0 0 0; font-size: 14px; line-height: 1.6;">
                                                <strong style="color: #111827;">⏰ This link will expire in 1 hour.</strong><br>
